...
/**
 * QQ表情适配器
 * 作者：janabo on 2017/3/17 17:11
 */
public class EmoticionAdapter extends BaseAdapter {
    private Context mContext = null;
    private float oldLocationX = -1;
    private float oldLocationY = -1;
    private KeyBoardListener mListener = null;
    private List<Emoticion> listEmoji = null;

    public EmoticionAdapter(Context context, List<Emoticion> mDatas, KeyBoardListener listener) {
        mContext = context;
        mListener = listener;
        listEmoji = mDatas;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final Emoticion emoji = listEmoji.get(position);
        try {
            Bitmap mBitmap = BitmapFactory.decodeStream(mContext.getAssets().open("face/emoticon/" + emoji.name + ".png"));
        ...
        holder.itemIvEmoji.setOnTouchListener(new View.OnTouchListener() {
            @Override
...
//                        if (oldLocationX == event.getX() && oldLocationY == event.getY()) {
                        if (mListener != null) {
                            if (emoji.name.equals("del_normal")) {
                                ...
                                mListener.selectedEmoticion(emoji);
                            }
                        }
//                        }
                        if (emoji.name.equals("del_normal")) {
